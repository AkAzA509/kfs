...
static void print_alloc(t_zone *zone, char *type, size_t *total_mem)
{
	...
	if (block) {
		...
		while (block) {
			printf(BLD_WHITE
			       "Block : %p -> %p = %lu bytes, the block is %s\n" RESET,
			       block, (char *)(block + 1) + block->size,
			       block->size,
			       block->is_free == true ? "free" : "in use");
...
t_frag_stats compute_fragmentation()
{
	...
	if (stats.total_free > 0)
		stats.ext_frag_percent =
			(100 * (stats.total_free - stats.largest_free)) /
			stats.total_free;
...
static void print_hex_dump(t_zone *zone, char *type, size_t print_limit)
{
	...
	if (block) {
		printf(BLD_RED "\n%s ZONE : %p - bloc in this zone %lu\n" RESET,
		       type, zone, zone->total_blocks);
		printf(BLD_WHITE
		       "------------------------------------------------------------------------------------\n" RESET);
		while (block) {
			printf(BLD_BLUE
			       "Owner zone: %s  -  bloc %s  -  size: %lu\n\n" RESET,
			       type, block->is_free == true ? "free" : "in use",
			       block->size);
			if (block->size < print_limit) {
				unsigned char *p = (unsigned char *)(block + 1);
				for (size_t i = 0; i < block->size; i += 16) {
					printf(BLD_WHITE "%p : " BLD_GREEN,
					       (char *)(block + 1) + i);
				...
				printf(BLD_WHITE
				       "The bloc is too large to be dump\n" RESET);
			printf(BLD_WHITE
			       "\n------------------------------------------------------------------------------------\n" RESET);
			block = block->next;
		}
