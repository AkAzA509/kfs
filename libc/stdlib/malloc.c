...
void perturb_fill(void *ptr, size_t len, bool inverted)
{
	...
	u32_t state = ((u32_t)(uintptr_t)ptr) ^ ((u32_t)len << 1) ^
		      ((u32_t)seed << 24) ^ 0x9E3779B9u;
	size_t i = 0;
	unsigned char byte;
			    ...
			    e_block_kind kind, t_zone **zone_head)
{
		...
		*zone_head = create_zone(zone_size, kind);
		if (!*zone_head) {
			if (g_alloc.env.MALLOC_LOG_)
				printf(BLD_RED
				       "Error: mmap() failed at %s zone creation" RESET,
				       kind == TINY ? "TINY" : "SMALL");
			return NULL;
		}
	}
	...
	while (zone) {
		if (zone->free_blocks > 0) {
			...
			while (block &&
			       (!block->is_free || block->size < align_mem))
				block = block->next;
			if (block)
				break;
		}
...
static void *large(size_t zone_large_size, t_zone **zone_head)
{
	...
	if (g_alloc.env.MALLOC_MMAP_MAX_ENABLE_)
		can_mmap = (g_alloc.env.mmap_max <
			    g_alloc.env.MALLOC_MMAP_MAX_VALUE_);
	...
	if (can_mmap) {
		...
		if (!zone) {
			if (g_alloc.env.MALLOC_LOG_)
				printf(BLD_RED
				       "Error: mmap() failed at LARGE zone creation" RESET);
			return NULL;
		}
...
void *internal_malloc(size_t size)
{
	...
	if (g_alloc.env.MALLOC_MMAP_THRESHOLD_ENABLE_ &&
	    align_mem >= g_alloc.env.MALLOC_MMAP_THRESHOLD_VALUE_)
		return large(LARGE_ZONE_SIZE(size), &g_alloc.large);

	if (align_mem <= TINY_MAX)
		return allocate_block(align_mem, TINY_ZONE_SIZE, TINY,
				      &g_alloc.tiny);
	else if (align_mem <= SMALL_MAX)
		return allocate_block(align_mem, SMALL_ZONE_SIZE, SMALL,
				      &g_alloc.small);
	else
		return large(LARGE_ZONE_SIZE(size), &g_alloc.large);
