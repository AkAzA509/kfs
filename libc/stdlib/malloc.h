...
#define TINY_ZONE_SIZE                 \
	ALIGN_TO_PAGE(sizeof(t_zone) + \
		      (ALIGN8(TINY_MAX) + sizeof(t_block)) * 100)
#define SMALL_ZONE_SIZE                \
	ALIGN_TO_PAGE(sizeof(t_zone) + \
		      (ALIGN8(SMALL_MAX) + sizeof(t_block)) * 100)
#define LARGE_ZONE_SIZE(x) \
	ALIGN_TO_PAGE(sizeof(t_zone) + (ALIGN8(x) + sizeof(t_block)))
...
typedef enum e_block_kind { TINY, SMALL, LARGE } e_block_kind;
